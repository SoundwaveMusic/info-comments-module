...
function getFollowers(num) {
  num = num - (num % 100);
  var point = (num % 1000 === 0) ? "" : `.${num % 1000}`;
  num = num - (num % 1000);
  return num + point;
}
...
function ArtistInfo(props) {
  ...
  return (
    <div>
      <div className="flex-container-artist">
        <div><img className='artistPic' src={props.pic}></img></div>
        ...
        <div className='stats-artist'>
          ...
          <div className='fol-stats'>{getFollowers(props.followers)}</div>
          <div>
          ...
          <div className='fol-stats'>{props.tracks}</div>
